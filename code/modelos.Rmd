...
experiencia[is.na(experiencia)] <- 0
experiencia <- experiencia +1

genero <- datos$genero
genero <- factor(datos$genero)
genero <- droplevels(genero)
genero <- as.integer(genero)


dependientes <- datos$dependientes
dependientes[is.na(dependientes)] <- 0
dependientes <- dependientes +1
...
edo_civil[is.na(edo_civil)] <- 0
edo_civil <- edo_civil +1

escolaridad <- datos$escolaridad 
escolaridad[is.na(escolaridad)] <- 0
escolaridad<- escolaridad +1

mes <- as.numeric(format(as.Date(datos$f_alta,format = "%d-%b-%y"), "%m"))
